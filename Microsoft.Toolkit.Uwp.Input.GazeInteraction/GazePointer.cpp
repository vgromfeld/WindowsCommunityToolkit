...
void GazePointer::IncrementDeviceReadyCount()
{
	_deviceReadyCount++;

	if (_deviceReadyCount == 1)
	{
		IsDeviceReadyChanged(nullptr, nullptr);
	}
}

void GazePointer::DecrementDeviceReadyCount()
{
	_deviceReadyCount--;
...
{
	auto device = GazeDevice::Create(this, args->Device);
	if (device != nullptr)
	{
...
void GazePointer::OnDeviceRemoved(GazeDeviceWatcherPreview^ sender, GazeDeviceWatcherRemovedPreviewEventArgs^ args)
{
	...
	{
		_devices->GetAt(index)->Destroy(this);
