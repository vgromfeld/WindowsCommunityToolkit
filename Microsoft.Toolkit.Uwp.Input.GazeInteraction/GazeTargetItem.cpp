...
Popup^ GazeTargetItem::_invokePopup = nullptr;
Rectangle^ GazeTargetItem::_invokeRectangle;
			...
			if (isInvoke && (_invokePopup == nullptr || !_invokePopup->IsOpen))
			{
				if (GazeInput::GetIsInvokeAnimationEnabled(control))
				{
					if (_invokePopup == nullptr)
					{
						...
						_invokeRectangle = ref new Rectangle();
						_invokeRectangle->IsHitTestVisible = false;

						_invokePopup->Child = _invokeRectangle;
					}

					_invokeRectangle->Fill = GazeInput::DwellFeedbackProgressBrush;
					_invokeRectangle->Opacity = 0.0;
					_invokeRectangle->Width = bounds.Width;
					_invokeRectangle->Height = bounds.Height;
					...
					Media::Animation::Storyboard^ storyboard = ref new Media::Animation::Storyboard();
					storyboard->Children->Append(animationObj);
					storyboard->SetTargetProperty(storyboard, "(UIElement.Opacity)");
					storyboard->SetTarget(storyboard, _invokeRectangle);
